...
**Gotcha:** `extradisplay uninstall` removes the LaunchAgent plist and runs `launchctl bootout`, but the app launched via `open -a` is managed by Launch Services, not launchd. It keeps running. `UninstallCommand` now calls `pkill -f ExtradisplayApp.app` first. Always verify no running instance before reinstalling.
...
**Date:** 2026-03-14
**Context:** First install used `sudo`, spawning `extradisplay daemon` as root. It created `/tmp/extradisplay.log` owned by root. User-mode writes and truncation both failed with "permission denied".
**Gotcha:** Use `~/Library/Logs/extradisplay.log` (always user-owned) not `/tmp/extradisplay.log`. If the log ends up root-owned: `sudo rm /tmp/extradisplay.log`. LaunchAgent plist now writes to `NSHomeDirectory()/Library/Logs/` which is always the invoking user's directory.
...
**Gotcha:** Before each install in an e2e loop: `pkill -f ExtradisplayApp.app 2>/dev/null || true; sleep 1; : > ~/Library/Logs/extradisplay.log 2>/dev/null || true`. Without this, open-a silently no-ops (app already running) and log assertions read stale data.
